...
CASES = [
    # ── Case 1：抓住必赢点（有较强干扰）─────────────────────────────────────
    # 黑 2 子 (2,1)(2,3) + 白 2 子 (0,0)(0,1)，黑白各 2 子合法
    # 黑下 (2,2) 横向三连直接赢；白两连 (0,0)(0,1) 制造干扰
    {
        "name": "winning_move",
    ...
    # ── Case 2：封堵对手必赢位（黑白子数对称）────────────────────────────────
    # 黑 2 子 (0,0)(2,2) + 白 2 子 (1,0)(1,1)，黑白各 2 子合法，轮到黑
    # 白下一步可在 (1,2) 三连赢，黑必须封堵
    {
        "name": "block_opponent",
        "desc": "白两连 (1,0)(1,1) + 黑 (0,0)(2,2)，黑必须堵 (1,2) 防白三连",
        "board": make_board(black=[(0,0),(2,2)], white=[(1,0),(1,1)]),
        "player": 1,
        "target": (1, 2),
        "target_desc": "封白制胜位",
    },
    # ── Case 3：抓住必赢点（较弱干扰）───────────────────────────────────────
    # 黑 2 子 (2,1)(2,3) + 白 1 子 (0,0)，黑多一子需黑先补白（合理：黑先走了 2 步，白 1 步）
    # 与 Case 1 相同的必赢局面，但白棋威胁更弱，用于对比"干扰强弱"的影响
    {
        "name": "winning_move_weak_threat",
        "desc": "黑间隔两连 (2,1)(2,3) + 白单子 (0,0)，黑下 (2,2) 三连赢（弱干扰版）",
        "board": make_board(black=[(2,1),(2,3)], white=[(0,0)]),
        "player": 1,
        "target": (2, 2),
        "target_desc": "三连必赢（无强威胁干扰）",
    },
]
